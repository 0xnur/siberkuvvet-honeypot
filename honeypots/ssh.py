'''
https://github.com/idleninja/ssh_honeypot-ish/blob/master/simplesshserver_creds.py
'''
#!/usr/bin/env python
...
SERVER_RSA_PRIVATE = '/root/another-simple-honeypot/honeypots/id_rsa'
SERVER_RSA_PUBLIC = '/root/another-simple-honeypot/honeypots/id_rsa.pub'

# Path to RSA SSH keys accepted by the server.
CLIENT_RSA_PUBLIC = '/root/another-simple-honeypot/honeypots/id_rsa.pub'
...
class myClass(userauth.SSHUserAuthServer, object):
    def auth_password(self, packet):
        ...
        user = self.user
        password = getNS(packet[1:])[0]
        self.logIt(user,password)
        super(myClass, self).auth_password(packet)

    def logIt(self, user, password):
        f = open('/var/log/honeypots/ssh.log', 'a')
